...
* for 0 ~ page_entries-1: valid bit: MSB (1: invlaid), dirty bit: LSB (1: dirty), bit 4 ~ 8: pid
*/
__device__ void init_invert_page_table(VirtualMemory *vm) {
    for (int i = 0; i < vm->PAGE_ENTRIES; i++) {
        vm->invert_page_table[i] = 0x80000000; // set valid bit & dirty bit
        vm->invert_page_table[i] |= (vm->PID) << 4; // set the pid
...
__device__ void vm_init(VirtualMemory* vm, uchar* buffer, uchar* storage,
    ...
    int PAGE_ENTRIES, uchar threadId) {
  // init variables
  vm->buffer = buffer;
  vm->storage = storage;
...
__device__ uchar vm_read(VirtualMemory *vm, u32 addr) {
    ...
    for (int i = 0; i < size; i++) { // traverse the page entries to find the matching VPN
        if (vm->invert_page_table[i + size] == VPN && (vm->invert_page_table[i]&0x000000F0)>>4 == vm->PID) { // if there is a match
    ...
    // page replacement (update the page table, set the PID)
...
__device__ void vm_write(VirtualMemory *vm, u32 addr, uchar value) {
    ...
    for (int i=0; i < size; i++) { // traverse the page table to find matching VPN
        if (vm->invert_page_table[i + size] == VPN && (vm->invert_page_table[i] & 0x000000F0) >> 4 == vm->PID) { // there is matching VPN
            if (vm->invert_page_table[i]>>31) {// invalid
                *(vm->pagefault_num_ptr) += 1;
...
__device__ int ReplacePage(VirtualMemory* vm, int VPN) {
    *(vm->pagefault_num_ptr) += 1;
    ...
    vm->invert_page_table[LRU_physicalIndex] |= vm->PID << 4; // set the pid
