...
struct dataIn { // data structure to be used in arithmetic
    char a;
    int b;
    short c;
};
...
int arithmetic(int fd, char operator, int operand1, short operand2)
{
    ...
    // 1st result: perform in the user program
    switch(data.a) {
        case '+':
            ans=data.b+data.c;
            break;
    ...
    // 2nd result: perform in kernel, blocking
    /******************Blocking IO******************/
    printf("Blocking IO\n");
    ret = 1;
    ...
    read(fd, &ret, sizeof(int)); // wait for read to execute

    printf("ans=%d ret=%d\n\n", ans, ret);
    /***********************************************/

    // 3rd result: perform in kernel, non-blocking
    /****************Non-Blocking IO****************/
    ...
    if(readable==1){
        printf("Can read now.\n");
        read(fd, &ret, sizeof(int)); // perform read until readable is set
    }
    printf("ans=%d ret=%d\n\n", ans, ret);
    /***********************************************/
...
int main()
{
    ...
    ret = 119010104;
    if (ioctl(fd, HW5_IOCSETSTUID, &ret) < 0) { // set student ID
        printf("set stuid failed\n");
        return -1;
    }

    ret = 1;
    if (ioctl(fd, HW5_IOCSETRWOK, &ret) < 0) { // print OK if complete the r/w function
        printf("set rw failed\n");
        return -1;
    }

    ret = 1;
    if (ioctl(fd, HW5_IOCSETIOCOK, &ret) < 0) { // print OK if complete ioctl function
        printf("set ioc failed\n");
        return -1;
    }

    ret = 1;
    if (ioctl(fd, HW5_IOCSETIRQOK, &ret) < 0) { // print OK if complete bonus
